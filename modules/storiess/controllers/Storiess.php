<?php
class Storiess extends Trongate {
    ...
    function init_gen_image() {
        ...
        //figure out what the main target subject
        ...
        //update the picture colums on the db
        $data['picture'] = $output_img->text;
        $update_id = (int) $update_id;
        $this->model->update($update_id, $data, 'storiess');

        $pic_path = $output_img->text;
        http_response_code(200);
        echo $pic_path;

        //echo 'You posted: '.$update_id.' and headline of: '.$headline;
    }

    function make_funny_headline($headline) {

        //$headline = 'Mona Lisa destroyed in fire';
        $ch = curl_init();
...
//json($output, true);
    ...
    function create() {
        ...
        if ($update_id>0) {
            $data['headline'] = 'Update Stories Record';
            $data['cancel_url'] = BASE_URL.'storiess/show/'.$update_id;
        } else {
            $data['headline'] = 'Create New Stories Record';
            $data['cancel_url'] = BASE_URL.'storiess/manage';
        }

        $data['form_location'] = BASE_URL.'storiess/submit/'.$update_id;
    ...
    function manage() {
        ...
        $pagination_data['pagination_root'] = 'storiess/manage';
        $pagination_data['record_name_plural'] = 'storiess';
    ...
    function show() {
        ...
        if ($data == false) {
            ...
            $data['headline'] = 'Stories Information';
            $data['view_file'] = 'show';
            $this->template('admin', $data);
        }
    }
    ...
    function submit() {
        ...
        if ($submit == 'Submit') {
            ...
            $this->validation_helper->set_rules('funny_headline', 'Funny Headline', 'required|min_length[2]');
            $this->validation_helper->set_rules('picture', 'Picture', 'required|min_length[2]');
            ...
            if ($result == true) {
                ...
                if ($update_id>0) {
                    ...
                    $update_id = $this->model->insert($data, 'storiess');
                    $flash_msg = 'The record was successfully created';
                }

                set_flashdata($flash_msg);
                redirect('storiess/show/'.$update_id);
    ...
    function submit_delete() {
        ...
        if (($submit == 'Yes - Delete Now') && ($params['update_id']>0)) {
            ...
            $params['module'] = 'storiess';
            $this->model->query_bind($sql, $params);

            //delete the record
            $this->model->delete($params['update_id'], 'storiess');
    ...
    function _get_data_from_db($update_id) {
        $record_obj = $this->model->get_where($update_id, 'storiess');
